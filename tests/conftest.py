...
# Scope hierarchy:
#   session  → created once per test run (DB engine, async event loop)
#   function → created fresh per test (DB session with rollback, mock services)

# Database strategy: each test runs in a transaction that is rolled back after.
# No test data persists between tests. No need to truncate tables.

# External dependencies (LLM, Docling, Langfuse) are always mocked.
# Only repositories run against a real test DB (configured via TEST_DATABASE_URL).
# """
...
# # ── Test database URL ─────────────────────────────────────────────────────────
# # Uses same DB as app by default. Override with TEST_DATABASE_URL env var.
...
#     """Create async engine and all tables once per test session."""
...
#     """Per-test DB session with automatic rollback. No data persists between tests."""
...
#     """Async HTTP client with DB session override for FastAPI dependency injection."""
...
# @pytest.fixture
# def mock_extractor():
#     """Mock BaseDocumentExtractor — returns fixed ExtractedDocument."""
...
# @pytest.fixture
# def mock_matcher():
#     """Mock BaseResumeMatcher — returns fixed EvaluationResult."""
#     mock = AsyncMock()
#     mock.evaluate.return_value = EvaluationResult(
#         overall_score=82,
#         verdict="shortlist",
#         skill_match=SkillMatch(score=85, matched=["Python", "FastAPI"], missing=["Docker"], partial=[], notes="Good match"),
...
# @pytest.fixture
# def mock_cache():
#     """Mock BaseCacheClient — always cache miss (returns None)."""
#     mock = AsyncMock()
...
#     """Minimal valid-ish PDF bytes for file upload tests."""
#     return b"%PDF-1.4\n1 0 obj\n<< /Type /Catalog >>\nendobj\nPython developer CV"
